...
def update_records():
    ...
    for record in records:
        ...
        # Always update with one randomly selected variant for each field
        updates = {
            "subject line": random.choice(subject_variants),
            "opener variant": random.choice(opener_variants),
            "closer variant": random.choice(closer_variants)
        }

        airtable.update(record_id, updates)
        print(f"✅ Updated record {record_id} with new variants")
        count += 1

    print(f"\n🎯 Done. {count} records updated with single variants.")
