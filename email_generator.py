...
paragraph_2_variants = [
    ...
    "I know {company} runs a layered, solution-based operation, which is exactly what our videos are built for: distilling complex offers into something people understand right away."
    "If you've ever tried to put {company}'s diverse offerings into words and ended up with a huge wall of text, you're not alone. We help simplify it with short, clear explainers; and we handle every step in-house, so you can just hit play.",
    ...
    "A lot of businesses like {company} who offer multi-layered service bundles struggle with the *'how do we explain this?'* problem. That’s where we come in. One short video often does the trick, and the best part is that we handle all the moving parts with zero effort on your end.",
